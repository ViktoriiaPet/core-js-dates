...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const finTime = new Date(dateEnd);
  const rest = (finTime.getTime() - startTime.getTime()) / 86400000 + 1;
  return rest;
}
...
function isDateInPeriod(date, period) {
  ...
  const end = new Date(period.end).setHours(0, 0, 0, 0);
...
function formatDate(date) {
  ...
  return new Intl.DateTimeFormat('en-US', typingDate).format(new Date(date));
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const daysInMonth = new Date(year, monthNew + 1, 0).getDate();

  for (let day = 1; day <= daysInMonth; day += 1) {
    const date = new Date(year, monthNew, day);
